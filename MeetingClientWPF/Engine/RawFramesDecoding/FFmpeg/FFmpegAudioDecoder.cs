...
namespace MeetingClientWPF.Engine.RawFramesDecoding.FFmpeg
{
    class FFmpegAudioDecoder
    {
        ...
        /// <exception cref="DecoderException"></exception>
        public bool TryDecode(RawAudioFrame rawAudioFrame)
        {
            if (rawAudioFrame is RawAACFrame aacFrame)
            {
                ...
                if (!_extraData.SequenceEqual(aacFrame.ConfigSegment))
                {
                    ...
                    unsafe
                    {
                        ...
                        {
                            int resultCode = FFmpegAudioPInvoke.SetAudioDecoderExtraData(_decoderHandle,
                                (IntPtr) extradataPtr, aacFrame.ConfigSegment.Count);

                            if (resultCode != 0)
                                throw new DecoderException(
                                    $"An error occurred while setting audio extra data, {_audioCodecId} codec, code: {resultCode}");
                        }
                    }
                }
            }
            ...
            unsafe
            {
                fixed (byte* rawBufferPtr = &rawAudioFrame.FrameSegment.Array[rawAudioFrame.FrameSegment.Offset])
                {
                    ...
                    if (_currentFrameFormat.SampleRate != sampleRate ||
                        _currentFrameFormat.BitPerSample != bitsPerSample ||
